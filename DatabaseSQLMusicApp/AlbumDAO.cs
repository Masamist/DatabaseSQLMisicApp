...
namespace DatabaseSQLMusicApp
{
    ...
    {
        // version only 1 contains fake data.
        ...
        public List<Album> getAllAlbums()
        {
            ...
            //difine the sql statement to fetch all albums
        ...
        public List<JObject> getTracksUsingJoin(int albumID)
        {
            ...
            //difine the sql statement to fetch all albums
            MySqlCommand command = new MySqlCommand();
            command.CommandText = "SELECT tracks.ID as trackID, `track_name`, `video_url`, `lyrics` FROM `tracks` JOIN albums ON albums_ID = albums.ID WHERE albums_ID = @albumid";
            command.Parameters.AddWithValue("@albumid", albumID);
            command.Connection = connection;
